...
def get_data():
    ...
    # 循环次数iter(这个设置的越大肯定是越精确), 此参数如果设置很大就可以防止过拟合问题，如果设置很小)
    model = ALS.train(dataRDD, 5, 20, 0.1)
    ...
    for i in range(1, total_user):
        ...
        for item in RecommendMovie:
            element = {"userId": item[0], "movieId": item[1], "movieName": movieName[item[1]], "rating": float(item[2])}
            list_data.append(element)

    pd_df = pd.DataFrame(list_data)

    # 将pd的DataFrame转换成spark的DataFrame
    spark_df = spark.createDataFrame(pd_df, schema=["userId", "movieId", "movieName", "rating"])
    # 给spark_df新增一列索引
